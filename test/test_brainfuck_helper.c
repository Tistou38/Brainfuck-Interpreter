...
{
    FILE *test_file = fopen("test/tmp/test_input_prog.bf", "w");
    fprintf(test_file, "[ This is a test comment],>");
    fclose(test_file);

    char *result = get_input_prog("test/tmp/test_input_prog.bf");
    const char *expected = "[],>";
...
void test_free_input_prog()
{
    ...
    FILE *test_file = fopen("test/tmp/test_free_input_prog.bf", "w");
    fprintf(test_file, "[ This is a file generated by test_brainfuck_helper.c to test free_input_prog],>");
    fclose(test_file);
    char *ip = get_input_prog("test/tmp/test_free_input_prog.bf");
...
{
    FILE *test_file =
        fopen("test/tmp/test_execute_instruction.bf", "w");
    fprintf(test_file,
            "[This is a file generated by test_brainfuck_helper.c to test execute_instruction]\n,.>++[-]<");
    fclose(test_file);

    char *input_prog =
        get_input_prog("test/tmp/test_execute_instruction.bf");
    struct Loops *loops = build_loops(input_prog);
    char *ip = input_prog;
