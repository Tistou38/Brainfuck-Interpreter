...
void test_free_input_prog()
{
    ...
    TEST_ASSERT_NOT_EQUAL_size_t(mallinfo_before.keepcost,
    ...
    TEST_ASSERT_EQUAL_size_t(mallinfo_before.keepcost,
                             mallinfo_after.keepcost);
}
...
{
    FILE *test_file =
        fopen("test_bf_files/test_execute_instruction.bf", "w");
    fprintf(test_file,
            "[This is a file used in test_brainfuck_helper.c to test execute_instruction]\n,.>++[-]<");
    fclose(test_file);

    char *input_prog =
        get_input_prog("test_bf_files/test_execute_instruction.bf");
    struct Loops *loops = build_loops(input_prog);
    char *ip = input_prog;
